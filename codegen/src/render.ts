import { buildSchema, GraphQLEnumType, Kind, OperationTypeNode, parse, visit } from "graphql";
import prettier from "prettier";
import type { Code } from "ts-poet";

import { selectorInterfaceTransform, typeTransform } from "./transforms";
...
export const render = async (sdl: string): Promise<string> => {
  ...
  `
    + results
      .flatMap((result) => result.definitions.map((code) => code.toString()))
      .join("\n");
